...
image_size = (256, 192)     # image input size
...
class EMBEDMammoDataModule(pl.LightningDataModule):
    def __init__(self, data_dir, csv_file, image_size, test_percent, val_percent, batch_alpha, batch_size, num_workers):
        ...
        self.data['density_label'] = self.data['biased_target']
...
def save_train_set(datamodule, output_fname):
    df = datamodule.train_data
    df.to_csv(output_fname, index=False)

def save_val_set(datamodule, output_fname):
    df = datamodule.val_data
    df.to_csv(output_fname, index=False)
...
def main(hparams):
    ...
    # ckpt = torch.load('/vol/biomedic3/es525/mammo-net/output-density-labelbias/resnet18-b32-fixed-alpha-1.0-ffdm-lr-0.00001-binary-hologic_c_30_undersampled/version_0/checkpoints/epoch=1-step=7794.ckpt')['state_dict']
    ...
    save_train_set(datamodule=data, output_fname=os.path.join(output_dir, 'train.csv'))
    save_val_set(datamodule=data, output_fname=os.path.join(output_dir, 'val.csv'))
